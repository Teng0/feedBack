...
#             form = forms.ReviewForm(request.POST)
#             form.save()
#             return render(request,'reviews/thank_you.html',{'form':form})
#         return render(request,'reviews/index.html',{'form':form})
...
class SingleReviewView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        context= super().get_context_data(**kwargs)
        loaded_reviw = self.object
        request = self.request
        favorite_id = request.session.get('favorite_review')

        context["is_favorite"] = str(favorite_id) == str(loaded_reviw.id)
...
def index(request):
     if request.method == "POST":
         ...
         form = forms.ReviewForm(request.POST,instance=existing_data)
         if form.is_valid():
